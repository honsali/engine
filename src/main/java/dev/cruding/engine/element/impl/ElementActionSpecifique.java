...
public class ElementActionSpecifique extends Element {


    private static final String utype = "Action";
    private BoutonActionSpecifique composantAction;

    public ElementActionSpecifique(BoutonActionSpecifique boutonActionSpecifique) {
    ...
    public void print(Page page) {
        ...
        /* *********************************************************************** */
        boolean actionTypeConnu = Action.standardActionList.indexOf(composantAction.ltype) > -1;
        ...
        if (composantAction.byEntity) {
            f.__("{ ", composantAction.entity.lname, " }");
        }
        f.__(") => {");
        f.flushInitBloc(page);
        f.L("");
        f.L____("const ", composantAction.lname, " = () => {");
        f.L________("execute(Ctrl", page.uc, ".", composantAction.lname);
        if (this.composantAction.byForm) {
            f.__(", { form }");
        }
        if (composantAction.byEntity) {
