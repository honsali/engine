...
public class ActionListerParIdPere extends Action {
    ...
    public void actionnable(Actionnable actionnable) {
        ...
        this.actionnable.lcoreName("listerParId" + entite().pere.uname);
        this.actionnable.lname("lister" + entite().uname + "ParId" + entite().pere.uname);
    };
    ...
    public void addRepositoryDeclaration(JavaFlow f) {
        f.L("");
        f.L____("List<", entite().uname, "> findAllBy", entite().upere, "_IdOrderBy", orderBy(), "(Long id", entite().upere, ");");
    }
    ...
    public void addResourceDeclaration(JavaFlow f) {
        f.L("");
        f.L____("@GetMapping(\"/", lcoreName(), "/{id", entite().upere, "}\")");
        f.L____("public List<", entite().uname, "Dto> ", lcoreName(), "(@PathVariable Long id", entite().upere, ") {");
        f.L________("return ", entite().lname, "Repository.findAllBy", entite().upere, "_IdOrderBy", orderBy(), "(id", entite().upere, ").stream().map(", entite().uname, "Dto::asEntity).collect(Collectors.toList());");
        f.L____("}");
    }
