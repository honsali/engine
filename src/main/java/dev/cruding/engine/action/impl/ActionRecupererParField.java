...
public class ActionRecupererParField extends Action {

    public Field field;

    public ActionRecupererParField(Field field) {
        this.field = field;
    }
    ...
    public void actionnable(Actionnable actionnable) {
        ...
        this.actionnable.lname("recuperer" + entity().uname + "Par" + field.uname);
    };
    ...
    public void addCtrlImplementation(MCFlow f) {
        ...
        f.L____("resultat.", entity().lname, " = await Service", entity().uname, ".recupererPar", field.uname, "(");
        if (byGrandFatherId() && entity().haveGrandFather) {
        ...
        f.__("requete.", field.lname, entity().uname, ");");
        f.L("};");
    }
    ...
    public void addServiceImplementation(Flow f) {
        f.L("");
        f.L("const recupererPar", field.uname, " = async (", field.lname, entity().uname, ": string) => {");
        f.L____("const ", entity().lname, ": I", entity().uname, " = (await axios.get<I", entity().uname, ">(`${resourceUri}/${", field.lname, entity().uname, "}`)).data;");
