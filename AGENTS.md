...
- `EnginePaths`, à côté d'`App`, centralise `sourceRoot`, `modelPath` et `outputRoot`. Les chemins sont de la configuration, pas un état du modèle de génération : ni `Context` ni `LoaderUtils` ne les possèdent. Les tests qui changent `outputRoot` restaurent sa valeur après leur scénario.
